...
class app(ttk.Frame):
    def __init__(self, master =None): # master : tk.TK class
        ...
        self.waveBtn = ttk.Button(self.master, text="Load mtr [.txt]", command=self.loadMtr_ibw)
        self.waveBtn.grid(column=0, row = 0,columnspan=2, sticky=tk.EW ,padx=pad_x, pady = pad_y)
        self.rsBtn = ttk.Button(self.master, text="Load rs [.txt]", command = self.loadrs_ibw)
    ...
    def loadMtr_ibw(self):
        self.mtrPathIgor = filedialog.askopenfilename(filetypes=[('','*.ibw')],title = "Choose matrix data file (.ibw)")
        if(self.mtrPathIgor ==''):
            return 0
        
        self.MtrIgorBW = ibw.IgorWave()
        self.MtrIgorBW.load_file(open(self.mtrPathIgor,"rb"))
        self.Mtr = self.MtrIgorBW.to_numpy_array()
    ...
    def loadrs_ibw(self):
        self.rsPathIgor = filedialog.askopenfilename(filetypes=[('','*.ibw')],title = "Choose matrix data file (.ibw)")
        if(self.rsPathIgor ==''):
            return 0
        
        self.rsIgorBW = ibw.IgorWave()
        self.rsIgorBW.load_file(open(self.rsPathIgor,"rb"))
        self.rs = self.rsIgorBW.to_numpy_array().flatten()  
        # print(self.rs.shape)
        # print(self.rs)     
        self.rsLoad = True
    ...
    def saveCrrSpec(self):
        self.MtrFileName = self.mtrPathIgor[self.mtrPathIgor.rfind('/')+1:]
